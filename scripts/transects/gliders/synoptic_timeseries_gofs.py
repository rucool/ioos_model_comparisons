...
from ioos_model_comparisons.calc import depth_bin, lon180to360
from ioos_model_comparisons.models import gofs
...
#%%
# Load RTOFS model
ds = gofs(rename=True).sel(depth=slice(depth_min, depth_max))

#%%
# Convert glider lons to 
...
ax[1].set_ylim([100, 0])
ax[1].set_title('GOFS', fontweight='bold', fontsize=18)
ax[1].set_ylabel('Depth (m)', fontweight='bold', fontsize=14)
ax[1].set_xlabel('')

sname = sdir / f"{id}-synoptic_timeseries-temperature_gofs.png"
...
sname = sdir / f"{id}-synoptic_timeseries-salinity_gofs.png"
plt.savefig(sname, dpi=dpi, bbox_inches='tight', pad_inches=0.1)
plt.close()
...
#%%
# Load RTOFS model
ds = cmems(rename=True).sel(depth=slice(depth_min, depth_max))

#%%
# Convert glider lons to 

depths = np.arange(depth_min, depth_max+depth_freq, depth_freq)
...
ax[1].set_ylim([100, 0])
ax[1].set_title('Copernicus', fontweight='bold', fontsize=18)
ax[1].set_ylabel('Depth (m)', fontweight='bold', fontsize=14)
ax[1].set_xlabel('')

sname = sdir / f"{id}-synoptic_timeseries-temperature_cmems.png"
...
sname = sdir / f"{id}-synoptic_timeseries-salinity_cmems.png"
plt.savefig(sname, dpi=dpi, bbox_inches='tight', pad_inches=0.1)
plt.close()
...
# t0 = pd.Timestamp(2021, 8, 20, 0, 0, 0)
...
rtofs = xr.open_dataset("https://tds.marine.rutgers.edu/thredds/dodsC/cool/rtofs/rtofs_us_east_scraped").set_coords(['lon', 'lat']).sel(depth=slice(0, 1000))
...
# Load and munge GOFS
gofs = xr.open_dataset("https://tds.hycom.org/thredds/dodsC/GLBy0.08/expt_93.0", drop_variables="tau").sel(depth=slice(0,1000), lon=slice(glons[0], glons[1]), lat=slice(lats[0], lats[1])).rename(
...
am = xr.open_dataset("https://www.ncei.noaa.gov/thredds-coastal/dodsC/ncom_amseas_agg/AmSeas_Dec_17_2020_to_Current_best.ncd").sel(depth=slice(0,1000), lon=slice(glons[0], glons[1]), lat=slice(lats[0], lats[1])).rename(
    {
        "water_temp": "temperature", 
        "water_u": "u",
        "water_v": "v",
...
cmems = cmems.sel(depth=slice(0,1000), lon=slice(lons[0], lons[1]), lat=slice(lats[0], lats[1]))
...
for ctime in times:
    ...
    plt.savefig(f'{rstr}_transect_map_salinity_{ctime_save_str}.png', dpi=300, facecolor='w', transparent=False, bbox_inches='tight', pad_inches=0.1)
    plt.close()

    # Create map plot
    # ax = plot_map(trtofs["temperature"].sel(depth=0), cm.haline, (36, 36.4, .05), title="RTOFS", figsize=(30,20))
    # ax.plot(pts[:,0], pts[:,1], 'r-.', linewidth=4, transform=ccrs.PlateCarree())
    # plt.savefig(f'{rstr}_transect_map_salinity_{ctime_save_str}.png', dpi=300, facecolor='w', transparent=False, bbox_inches='tight', pad_inches=0.1)
    ...
    # %%
    plot_transect(rds, "temperature", cm.thermal, levels=levels_temperature, xaxis=xaxis, ax=axs['temperature'], cbar=dict(ax=axs['temperature'], orientation='vertical', pad=0.01), fontfrac=1.25, contour=[26])
    plot_transect(rds, "salinity", cm.haline, levels=levels_salinity, xaxis=xaxis, ax=axs['salinity'], cbar=dict(ax=axs['salinity'], orientation='vertical', pad=0.01), fontfrac=1.25, contour=[36.6])
    plot_transect(rds, "density", cm.dense, levels=levels_density, xaxis=xaxis, ax=axs['density'], cbar=dict(ax=axs['density'], orientation='vertical', pad=0.01), fontfrac=1.25, contour=[26])

...
prj = ccrs.Mercator(central_longitude=180)
...
floats1 = get_argo_floats_by_time([145, 180, -50, -5],
                                 date_start,
                                 date_end, 
                                 variables=vars)

floats2 = get_argo_floats_by_time([-180, -120, -50, -5],
                                 date_start,
                                 date_end, 
                                 variables=vars)

floats = pd.concat([floats1, floats2])
...
def process_argo(region):    
    ...
    for gname, df in argo_region.reset_index().groupby(['argo', 'time']):
        ...
        if not profile_exist:
            ...
            ax5 = fig.add_subplot(gs[1, -1], projection=prj)# Map
            ...
            cplt.create([145, 240, -50, -5], ax=ax5, bathymetry=False, proj=prj)

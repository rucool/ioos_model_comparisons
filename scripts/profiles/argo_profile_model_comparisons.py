...
for region in regions.items():
    ...
    for float in floats.platform_number.unique():
        ...
        x = temp['longitude (degrees_east)'].unique()[0]
        y = temp['latitude (degrees_north)'].unique()[0]
        f_time = temp['time (UTC)'].unique()[0]
        ...
        # Temperature
        ax[0].plot(temp['temp (degree_Celsius)'], temp['pres (decibar)'],
        ...
        # Salinity
        ax[1].plot(temp['psal (PSU)'], temp['pres (decibar)'],
                   label=f'{float} [{str(round(x, 2))}, {str(round(y, 2))}]')
        ...
        t_str = f_time.strftime('%Y-%m-%d %H:%M:%SZ')
        plt.suptitle(f'Argo #{float} Profile Comparisons\n'
                     f'ARGO: { f_time.strftime("%Y-%m-%d %H:%M:%SZ") }\n'
                     f'RTOFS: {pd.to_datetime(rtofs_sub.time.data)}\n'
                     f'GOFS: {pd.to_datetime(gofs_temp.time.data)}', fontsize=8)
        save_str = f'{f_time.strftime("%Y-%m-%dT%H%M%SZ")}-argo_{float}-model-comparison.png'

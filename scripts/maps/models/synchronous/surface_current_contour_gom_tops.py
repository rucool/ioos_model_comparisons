...
# ddir = Path('/Users/mikesmith/Documents/data/tops/')
...
search_start = yesterday - dt.timedelta(hours=conf.search_hours)
...
if conf.argo:
    argo_data = get_argo_floats_by_time(extent, search_start, yesterday)
else:
    argo_data = pd.DataFrame()

if conf.gliders:
    glider_data = get_active_gliders(extent, search_start, yesterday, parallel=False)
else:

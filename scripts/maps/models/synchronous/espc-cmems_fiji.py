"""
ESPC vs CMEMS side-by-side comparison maps for Fiji.
...
Fiji straddles the antimeridian (160°E–170°W = 160–190 in 0-360). CMEMS uses
-180/180 longitude, so data loading splits the domain into a west (160–180°E)
and east (-180 to -170°E = 180–190°E in 0-360) half, then concatenates.
"""
import matplotlib
...
# Fiji straddles the antimeridian so we use Mercator(central_longitude=180).
# This keeps the domain [160°E – 170°W] contiguous without splitting.
MAP_PROJECTION = ccrs.Mercator(central_longitude=180)
DATA_PROJECTION = ccrs.PlateCarree()

# ── Worker-level globals (populated by worker_initializer) ─────────────────
_worker_gds_ts = None
_worker_gds_uv = None
_worker_cmems_obj = None
...
def _init_fiji_axis(ax, extent, bathy=None, label_left=True, label_right=False):
    ...
    # Fiji location marker (Suva: 178.44°E, 18.14°S)
    h_marker, = ax.plot(
        178.44, -18.14,
    ...
    ax.set_xticks(np.arange(140, 195, 10), crs=DATA_PROJECTION)
    ax.set_xticks(np.arange(140, 195, 5), minor=True, crs=DATA_PROJECTION)
    ax.set_yticks(np.arange(-30, 0, 5), crs=DATA_PROJECTION)
    ax.set_yticks(np.arange(-30, 0, 5), minor=True, crs=DATA_PROJECTION)
...
def plot_fiji_comparison(ds_espc, ds_cmems, region,
    ...
    for var_key, var_list in region['variables'].items():
        ...
        for item in var_list:
            ...
            _init_fiji_axis(ax1, extent, bathy=bathy, label_left=True, label_right=False)
            _init_fiji_axis(ax2, extent, bathy=bathy, label_left=False, label_right=True)
...
def plot_fiji_streamplot(ds_espc_uv, ds_cmems, region,
                         bathy=None, argo=None, gliders=None,
...
def attempt_cmems_data_load_fiji(cmems_instance, ctime, extent):
    """
    Load CMEMS data for the Fiji domain, which crosses the antimeridian.

    CMEMS uses -180/180 longitude. The Fiji extent [160, 190, -26, -5] straddles
    180°, so we make two subset calls and concatenate:
      west:  lon 160–180   (standard)
      east:  lon -180–-170 (= 180–190°E), then shift lons by +360 to get 180–190

    The concatenated dataset has monotonically increasing lons in 0-360 space,
    matching the ESPC data and the Mercator(central_longitude=180) map projection.
    """
    try:
        ...
        if west is None and east is None:
            raise ValueError(f"No valid CMEMS data for time {ctime}.")

        parts = []
        if west is not None:
            parts.append(west)
        if east is not None:
            # Shift -180:-170 → 180:190 so lons are contiguous with the west part
            east = east.assign_coords(lon=(east['lon'] + 360))
            parts.append(east)

        data = xr.concat(parts, dim='lon') if len(parts) > 1 else parts[0]
        logger.info(f"CMEMS Fiji: Data loaded for time {ctime}.")
        return True, data

    except Exception as e:
        logger.warning(f"CMEMS Fiji: Data not available for time {ctime} - {e}")
        return False, None
...
def worker_initializer(
    ...
    global _worker_gds_ts, _worker_gds_uv, _worker_cmems_obj
...
def process_time(ctime):
    ...
    for region_key in config['regions']:
        ...
        cdt_flag = False
        cdt_data = None
        if config['plot_cmems'] and cmems_obj is not None:
            cdt_flag, cdt_data = attempt_cmems_data_load_fiji(cmems_obj, ctime, extent)

        # ── Platform subsets ───────────────────────────────────────────
        # Fiji argo/glider search uses -180/180 portion only (160–180°E);
        # data east of 180° is very sparse in these APIs.
        platform_extent = [139.75, 180, -30.25, -4.75]
        ...
        if not argo_data.empty:
            ...
            lat = argo_data['lat']
            mask = (
                (platform_extent[0] <= lon) & (lon <= platform_extent[1]) &
                (platform_extent[2] <= lat) & (lat <= platform_extent[3])
            )
        ...
        if gdt_flag and cdt_flag:
            try:
                plot_fiji_comparison(gdt_ts_data, cdt_data, region, **kwargs)
        ...
        if gdv_flag and cdt_flag:
            try:
                plot_fiji_streamplot(gdt_uv_data, cdt_data, region, **kwargs)
...
def main(parallel=True, max_workers=None):
    ...
    # For argo/gliders we use the -180/180 portion only (160–180°E) since those
    # APIs use standard longitude; data east of 180° is very sparse anyway.
    platform_extent = [139.75, 180, -30.25, -4.75]
    ...
    if conf.gliders:
        try:
            glider_data = get_active_gliders(platform_extent, search_start, date_end, parallel=False, timeout=60)
    ...
    if conf.bathy:
        try:
            bathy_data = load_or_fetch_bathymetry(platform_extent, cache_dir)

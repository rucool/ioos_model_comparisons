"""
OHC three-panel difference plots — real-time (today only).

Produces plots for the current UTC day at 00Z, 06Z, 12Z, 18Z.
Each timestep produces two figures:
  1. [RTOFS OHC | ESPC OHC  | RTOFS - ESPC]
  2. [RTOFS OHC | CMEMS OHC | RTOFS - CMEMS]

ESPC and CMEMS are regridded to the RTOFS curvilinear grid using xesmf.
"""
...
import pandas as pd
import xarray as xr
from ioos_model_comparisons.calc import lon180to360, lon360to180, compute_ohc_vectorized, regrid_to_rtofs
...
from ioos_model_comparisons.plotting import plot_ohc_three_panel
...
def worker_initializer(
    ...
    if config_dict['plot_rtofs']:
        rds_local = r()
        rds_local = rds_local[['temperature', 'salinity']]
    ...
    # ── ESPC (always FMRC best-forecast for real-time) ─────────────────
...
def process_time(ctime):
    ...
    if config['plot_espc'] and eds is not None:
        try:
            ...
            eds_time['lon'] = lon360to180(eds_time['lon'])
            edt_flag = True
        except KeyError:
            pass
    ...
    if config['plot_cmems'] and cds is not None:
        try:
            ...
            rename_map = {k: v for k, v in {'longitude': 'lon', 'latitude': 'lat'}.items() if k in cds_time.coords}
            if rename_map:
    ...
    for region in config['regions']:
        ...
        try:
            ohc_rtofs = _load_ohc(cache_dir, region, ctime, 'rtofs')
            if ohc_rtofs is None:
                ohc_rtofs = compute_ohc_vectorized(rds_slice)
                ohc_rtofs = ohc_rtofs.assign_coords(
                    lon=rds_slice['lon'], lat=rds_slice['lat']
                )
                _save_ohc(ohc_rtofs, cache_dir, region, ctime, 'rtofs')
                print(f"  Cached RTOFS OHC → {_ohc_cache_path(cache_dir, region, ctime, 'rtofs')}")
            else:
                print(f"  Loaded RTOFS OHC from cache")
            rtofs_lon2d = ohc_rtofs['lon'].values
            rtofs_lat2d = ohc_rtofs['lat'].values
        except Exception as err:
            print(f"  RTOFS OHC failed for {region} at {ctime}: {err}")
            continue

        ohc_espc = None
        if edt_flag:
            try:
                ohc_espc = _load_ohc(cache_dir, region, ctime, 'espc')
                if ohc_espc is None:
                    eds_slice = eds_time.sel(
                        lon=slice(extent_data[0] - 2, extent_data[1] + 2),
                        lat=slice(extent_data[2] - 2, extent_data[3] + 2),
                    )
                    print(f"  Loading ESPC data ({region})...")
                    _load_with_timeout(eds_slice)
                    ohc_espc_reg = compute_ohc_vectorized(eds_slice)
                    ohc_espc = regrid_to_rtofs(
                        ohc_espc_reg, rtofs_lon2d, rtofs_lat2d,
                        cache_key=(region, 'espc'),
                        weights_dir=cache_dir,
                    )
                    ohc_espc = ohc_espc.assign_coords(
                        lon=xr.DataArray(rtofs_lon2d, dims=['y', 'x']),
                        lat=xr.DataArray(rtofs_lat2d, dims=['y', 'x']),
                    )
                    _save_ohc(ohc_espc, cache_dir, region, ctime, 'espc')
                    print(f"  Cached ESPC OHC → {_ohc_cache_path(cache_dir, region, ctime, 'espc')}")
                else:
                    print(f"  Loaded ESPC OHC from cache")
            except Exception as err:
                print(f"  ESPC OHC/regrid failed for {region} at {ctime}: {err}")

        ohc_cmems = None
        if cdt_flag:
            try:
                ohc_cmems = _load_ohc(cache_dir, region, ctime, 'cmems')
                if ohc_cmems is None:
                    ...
                    ohc_cmems_reg = compute_ohc_vectorized(cds_slice)
                    ohc_cmems = regrid_to_rtofs(
                        ohc_cmems_reg, rtofs_lon2d, rtofs_lat2d,
                        cache_key=(region, 'cmems'),
                        weights_dir=cache_dir,
                    )
                    ohc_cmems = ohc_cmems.assign_coords(
                        lon=xr.DataArray(rtofs_lon2d, dims=['y', 'x']),
                        lat=xr.DataArray(rtofs_lat2d, dims=['y', 'x']),
                    )
                    _save_ohc(ohc_cmems, cache_dir, region, ctime, 'cmems')
                    print(f"  Cached CMEMS OHC → {_ohc_cache_path(cache_dir, region, ctime, 'cmems')}")
                else:
                    print(f"  Loaded CMEMS OHC from cache")
            except Exception as err:
                print(f"  CMEMS OHC/regrid failed for {region} at {ctime}: {err}")
        ...
        if ohc_espc is not None:
            plot_ohc_three_panel(ohc_rtofs, ohc_espc, comp_model_name='ESPC', **common_kwargs)
            plots_generated.append(f"{region} | OHC | RTOFS vs ESPC")

        if ohc_cmems is not None:
            plot_ohc_three_panel(ohc_rtofs, ohc_cmems, comp_model_name='COPERNICUS', **common_kwargs)
            plots_generated.append(f"{region} | OHC | RTOFS vs COPERNICUS")
...
def main(parallel=True, max_workers=None):
    ...
    # Take the last 4 timestamps (one full RTOFS daily run: 06Z, 12Z, 18Z, 00Z+1)
...
if __name__ == "__main__":
    main(parallel=True, max_workers=2)

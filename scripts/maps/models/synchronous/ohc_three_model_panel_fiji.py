"""
OHC two-model side-by-side plots for Fiji: ESPC | CMEMS.
...
Fiji straddles the antimeridian (148–190°E in 0-360 space).
ESPC is subsetted directly in 0-360.  CMEMS is loaded in two parts
(west 148–180, east −180–−170) and stitched into a contiguous 0-360 dataset.
Plots use Mercator(central_longitude=180) to keep the domain contiguous.
"""
import matplotlib
...
from ioos_model_comparisons.calc import lon180to360, lon360to180, compute_ohc_vectorized
...
# Fiji straddles the antimeridian — use central_longitude=180 throughout
FIJI_TRANSFORM = {
    'map': ccrs.Mercator(central_longitude=180),
    'data': ccrs.PlateCarree(),
}

# Platform search uses the -180/180 portion only (APIs don't handle 0-360)
...
# ── CMEMS antimeridian loader ──────────────────────────────────────────────

def _load_cmems_fiji(cmems_instance, ctime, extent):
    """
    Load CMEMS for the Fiji domain, which crosses the antimeridian.

    Two subset calls are made and concatenated:
      west: lon 148–180   (standard)
      east: lon -180 – lon360to180(extent[1])  (= -170 for extent[1]=190)
            then shift lons +360 to get 180–190

    Returns an xr.Dataset with monotonically increasing lons in 0-360 space,
    or None on failure.
    """
    try:
        lat_extent = extent[2:]
        lon_west = [float(extent[0]), 180.0]
        lon_east_max = float(lon360to180([extent[1]])[0])   # e.g. 190 → -170
        lon_east = [-180.0, lon_east_max]
        ...
        if west is None and east is None:
            raise ValueError(f"CMEMS returned None for both halves at {ctime}")

        parts = []
        if west is not None:
            parts.append(_rename_lon_lat(west))
        if east is not None:
            east = _rename_lon_lat(east)
            east = east.assign_coords(lon=(east['lon'] + 360))
            parts.append(east)

        data = xr.concat(parts, dim='lon') if len(parts) > 1 else parts[0]
        logger.info(f"  CMEMS Fiji: loaded for {ctime}")
        return data

    except Exception as e:
        logger.warning(f"  CMEMS Fiji load failed for {ctime}: {e}\n{traceback.format_exc()}")
        return None
...
def worker_initializer(
    ...
    global _worker_eds, _worker_cmems_obj
...
def process_time(ctime):
    ...
    for region_key in config['regions']:
        ...
        extent = configs["extent"]   # 0-360 space
        ...
        if config['plot_cmems'] and cmems_obj is not None:
            try:
                ...
                if ohc_cmems is None:
                    cds_time = _load_cmems_fiji(cmems_obj, ctime, extent_data)
                    if cds_time is None:
                        raise ValueError(f"CMEMS Fiji stitch returned None for {ctime}")
                    cds_slice = cds_time.sel(depth=slice(0, 400))
        ...
        # ── Platforms (148-180 portion only for API compatibility) ────
        argo = gliders = pd.DataFrame()
        if not argo_data.empty:
            lon = argo_data['lon']
            lat = argo_data['lat']
            mask = (
                (FIJI_PLATFORM_EXTENT[0] <= lon) & (lon <= FIJI_PLATFORM_EXTENT[1]) &
                (FIJI_PLATFORM_EXTENT[2] <= lat) & (lat <= FIJI_PLATFORM_EXTENT[3])
            )
...
def main(parallel=True, max_workers=None):
    ...
    logger.info(f"Global extent (0-360): {global_extent}")
    ...
    # ── Platform and bathymetry data (148-180 portion for API compatibility) ─
    argo_data = pd.DataFrame()
    if conf.argo:
        try:
            argo_data = load_or_fetch_argo(FIJI_PLATFORM_EXTENT, search_start, date_end, cache_dir)
        except Exception as e:
    ...
    if conf.gliders:
        try:
            glider_data = load_or_fetch_gliders(FIJI_PLATFORM_EXTENT, search_start, date_end, cache_dir)
        except Exception as e:

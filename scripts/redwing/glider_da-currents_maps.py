...
# Predefined regional extents [lon_min, lon_max, lat_min, lat_max].
# Listed smallest-first so select_region() returns the tightest fit.
REGIONS = [
    ...
    {'name': 'Tropical Western Atlantic',
     'extent': [-72, -40, 0, 27]},
    {'name': 'North Atlantic',           # broad fallback
     'extent': [-85, -10, 0, 60]},
]

# Geographic footprint of the Doppio ROMS model (approximate bounding box).
# Gliders inside this box get Doppio added to their comparison set.
DOPPIO_DOMAIN = [-82, -55, 25, 48]   # [lon_min, lon_max, lat_min, lat_max]

# Per-glider overrides: force a specific region and/or zoom instead of auto-selecting.
...
def union_extent(extents: list) -> list:
    """Return the bounding box union of multiple [lon_min, lon_max, lat_min, lat_max] extents."""
    return [
...
def select_region(lat: float, lon: float) -> dict:
    """Return the smallest predefined region containing (lat, lon).

    Falls back to the last (broadest) entry in REGIONS if none match.
    """
    containing = [
        r for r in REGIONS
        if r['extent'][0] <= lon <= r['extent'][1]
        and r['extent'][2] <= lat <= r['extent'][3]
    ]
    if not containing:
        logger.warning(
            "Glider at (%.2f, %.2f) is outside all predefined regions; "
            "using fallback '%s'.", lat, lon, REGIONS[-1]['name']
        )
        return REGIONS[-1]
    # Prefer the smallest area (tightest fit)
    return min(containing, key=lambda r: (
        (r['extent'][1] - r['extent'][0]) * (r['extent'][3] - r['extent'][2])
    ))
...
def load_doppio():
    """Return a Doppio instance (lazy; actual data fetched per .sel() call)."""
    try:
        from ioos_model_comparisons.models import Doppio
        dop = Doppio()
        logger.info("Doppio instance created successfully")
        return dop
    except Exception as exc:
        logger.error("Failed to create Doppio instance: %s", exc)
        return None


def glider_in_doppio_domain(lat: float, lon: float) -> bool:
    """Return True if the position falls within the Doppio model footprint."""
    return (
        DOPPIO_DOMAIN[0] <= lon <= DOPPIO_DOMAIN[1]
        and DOPPIO_DOMAIN[2] <= lat <= DOPPIO_DOMAIN[3]
    )
...
def plot_twa_map(
    ...
    # Tropical Western Atlantic is the historic default extent
    extent = region_extent if region_extent is not None else [-72, -40, 0, 27]
...
def process_and_plot_time(
    ...
    if doppio_obj is not None:
        try:
            logger.info("Doppio: fetching time slice %s", reference_time)
            dop_full = doppio_obj.sel(time=reference_time)
            extent_buf = expand_extent(extent, buffer=1.0)
            eta_sl, xi_sl = doppio_obj.subset_indices(extent_buf)
            dop_slice = dop_full.isel(y=eta_sl, x=xi_sl)
            dop_slice.attrs['model'] = 'Doppio'
            logger.info("Doppio: Processing")
            plot_twa_map(
                dop_slice,
                bathy=bathy,
                gliders=glider_data,
...
def main():
    ...
    logger.info("Starting TWA map plotting script")
    ...
    # ---- Fetch surfacings, check staleness, select region ----
    glider_info = []
    for gname, dep_id in glider_deployments.items():
        ...
        # Region: use override if specified, otherwise auto-select smallest containing box
        if "region" in override:
            region = next(
                (r for r in REGIONS if r["name"] == override["region"]),
                select_region(float(latest_pos['latitude']), float(latest_pos['longitude'])),
            )
            logger.info("Glider %s: using overridden region '%s'.", gname, region['name'])
        else:
            region = select_region(float(latest_pos['latitude']), float(latest_pos['longitude']))

        # Zoom: use override (fixed lon, lat centered on current position) or dynamic
        if "zoom" in override:
            oz     = override["zoom"]
            lat0   = float(latest_pos['latitude'])
    ...
    if not glider_info:
        logger.error("No active non-stale gliders to process; exiting.")
        return

    # ---- Reference date from most recently active glider ----
    reference_date = max(pd.Timestamp(g['data'].index.max()) for g in glider_info)

    # ---- Union extent for shared model loading ----
    all_extents = [g['region']['extent'] for g in glider_info]
    load_extent = expand_extent(union_extent(all_extents), buffer=1.0)
    logger.info("Union load extent: %s", load_extent)
    ...
    rtofs_data = espc_data = cmems_data = doppio_obj = None
    ...
    # Load Doppio once if any active glider falls inside its domain
    if models_enabled.get('plot_doppio', True):
        doppio_gliders = [
            g for g in glider_info
            if glider_in_doppio_domain(
                float(g['data'].iloc[-1]['latitude']),
                float(g['data'].iloc[-1]['longitude']),
            )
        ]
        if doppio_gliders:
            logger.info(
                "Gliders within Doppio domain: %s — loading Doppio.",
                [g['name'] for g in doppio_gliders],
            )
            doppio_obj = load_doppio()
        else:
            logger.info("No active gliders within Doppio domain; skipping Doppio.")
    ...
    for g in glider_info:
        ...
        # Only include Doppio for this glider if it's within the domain
        glider_doppio = (
            doppio_obj
            if glider_in_doppio_domain(
                float(gdata.iloc[-1]['latitude']),
                float(gdata.iloc[-1]['longitude']),
            )
            else None
        )

...
#
regions = limits_regions('gofs', ['nola'])
#
...
f = 'https://tds.hycom.org/thredds/dodsC/GLBy0.08/expt_93.0'

tC = dt.datetime(2021, 8, 27, 18, 0, 0)

with xr.open_dataset(f, drop_variables='tau') as ds:
    ds = ds.rename({'surf_el': 'sea_surface_height', 'water_temp': 'temperature', 'water_u': 'u', 'water_v': 'v'})
    lat = ds.lat.data
    lon = ds.lon.data

    t0 = pd.to_datetime(tC - dt.timedelta(hours=search_hours))
    t0_glider = pd.to_datetime(tC - dt.timedelta(hours=10000))
    t1 = pd.to_datetime(tC)
    kwargs['t0'] = t0
    ...
    for region in regions.items():
        ...
        glider = active_gliders(bbox=extent, time_start=t0_glider, time_end=t1, glider_id=glider_id)
        ...
        print(f'Accessing GOFS: {t1}')
        # tds = ds.sel(time=t1)  # Select the latest time
        sub = ds.sel(time=t1, lon=slice(extent[0] + 359, extent[1] + 361), lat=slice(extent[2] - 1, extent[3] + 1))
        ...
        for k, v in limits.items():
            ...
            for item in v:
                ...
                # plot entire track
                ax.plot(glider['longitude (degrees_east)'], glider['latitude (degrees_north)'], color='darkgray',
                ...
                for float in most_recent.itertuples():
                    if float.platform_number in rapid_floats:
                        ...
                        color = 'yellow'
                        # label = 'Standard'
                    ax.plot(float._4, float._5, marker='s', markersize=9, markeredgecolor='black', color=color, transform=projection['data'])
                    # map_add_legend(ax)

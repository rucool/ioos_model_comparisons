...
def read_hycom_fields(file_name: str, fields: list, layers=[], replace_to_nan=True, verbose=False):
    ...
    if len(layers) != 0:
        layers_per_field = {field: [layer for layer in layers if layer in range(
            num_layers[field])] for field in fields}
    else:
        layers_per_field = {field: range(
            num_layers[field]) for field in fields}

    # Create the dictionary that will contain the np arrays with the fields information
    np_fields = {field: np.zeros(
        (len(layers_per_field[field]), lat_size, lon_size)) for field in fields}
    ...
    if verbose:
        ...
        for field in fields:
            print(
                F"\tReading layers {layers_per_field[field]} for field {field}. Total layers: {num_layers[field]}")
    ...
    for field in fields:
        for cur_layer_idx, cur_layer in enumerate(layers_per_field[field]):
            ...
            a_file.seek(offset)
            cur_layer_data = np.fromfile(
                file_name+'.a', dtype='>f', count=layer_size, offset=offset)
            if replace_to_nan:
                cur_layer_data[cur_layer_data > NAN_TH] = np.nan
            np_fields[field][cur_layer_idx, :, :] = np.reshape(
                cur_layer_data, (lat_size, lon_size))
